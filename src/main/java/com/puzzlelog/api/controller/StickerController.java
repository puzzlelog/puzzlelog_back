...
public class StickerController {
    ...
    //스티커 추가 (관리자만 가능, multipart/form-data 지원)
    ...
    public ResponseEntity<ApiResponse<Sticker>> addSticker(
        ...
        try {
            Sticker savedSticker = stickerService.addSticker(name, type, file);
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<Sticker>> getStickerById(@PathVariable String id) {
        return stickerService.getStickerById(id)
                .map(sticker -> ResponseEntity.ok(ApiResponse.success(sticker, "스티커 조회 성공")))
                .orElse(ResponseEntity.status(404).body(ApiResponse.fail("스티커를 찾을 수 없습니다.")));
    }
